...
from analisis_periodicos import link_el_tiempo, link_el_espectador
...
def test_link_el_tiempo():

    soup = BeautifulSoup(html_tiempo, "html.parser")
    links = soup.find_all('a')[2]

    assert link_el_tiempo(links) == text_response_tiempo

def test_link_el_espectador():

    soup = BeautifulSoup(html_espectador, "html.parser")
    links = soup.find_all('a')[0]
    
    assert link_el_espectador(links) == text_response_espect